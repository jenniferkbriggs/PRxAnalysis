...
addpath('home/jenniferb/Git/UniversalCode/')
    ...
    for i = 1:30
        for j = 2:65
            time_hyper = squeeze(out.time(patdata).data(i,j,:));
            prx_hyper = squeeze(out.PRx(patdata).data(i,j,:));
            ...
            PRx_all(i,j,:) = interp1(time_hyper_unique, prx_hyper(unique_indx), time);
        end
    end
    ...
    %originally averaging over just the five common hyperparameters
   ...
   % nexttile,
    ...
    if 1 
        for i = 1:30
            for j = 2:65
                deviation_from_av_all(patdata,i,j,:) = (squeeze(PRx_all(i,j,:)) - all_av);
            end
        end
    end
    ...
    writetable(Uncertainty_in_common_estimators, '/data/brain/tmp_jenny/PRxError/Results/Uncertainty_in_common_estimators.csv')
    writematrix(squeeze(mean(total_uncertainty,3,'omitnan')), '/data/brain/tmp_jenny/PRxError/Results/STD_all.csv')
    ...
    writematrix(total_bias, '/data/brain/tmp_jenny/PRxError/Results/Mean_all.csv')

    %find optimal
    optimal_uncertainty = (uncertainty_mean < 0.02);
    writematrix(optimal_uncertainty, '/data/brain/tmp_jenny/PRxError/Results/STD_all_bin.csv')

    optimal_bias = (abs(total_bias) < 0.001);
    writematrix(optimal_bias, '/data/brain/tmp_jenny/PRxError/Results/Mean_all_bin.csv')
